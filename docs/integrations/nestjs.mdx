...
To define relations in models, use the corresponding methods. For example, to define that each `User` can have multiple photos, use the `@HasMany()` decorator.
...
> **Hint**: Mongoloquent Transaction only worked in the same connection and database. To learn more about Mongoloquent Transaction read [this](../database/getting-started/#database-transactions) chapter.
