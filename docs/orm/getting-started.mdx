...
Once you have created a model and `its associated database collection`, you are ready to start retrieving data from your database. You can think of each Mongoloquent model as a powerful [query builder](/docs/database/query-builder) allowing you to fluently query the database collection associated with the model. The model's `all` method will retrieve all of the records from the model's associated database collection:
...
The Mongoloquent `all` method will return all of the results in the model's table. However, since each Mongoloquent model serves as a [query builder](/docs/database/query-builder), you may add additional constraints to queries and then invoke the `get` method to retrieve the results:
...
> Since Mongoloquent models are query builders, you should review all of the methods provided by Mongoloquent's [query builder](/docs/database/query-builder). You may use any of these methods when writing your Mongoloquent queries.
...
In addition to the methods provided by Mongoloquent's base collection class, the Mongoloquent collection class provides [a few extra methods](#available-methods) that are specifically intended for interacting with collections of Mongoloquent models.
...
When interacting with Mongoloquent models, you may also use the `count`, `sum`, `max`, and other [aggregate methods](/docs/database/query-builder#aggregates) provided by the Mongoloquent [query builder](/docs/database/query-builder). As you might expect, these methods return a scalar value instead of an Mongoloquent model instance:
...
If you are utilizing [soft deleting models](/docs/orm#soft-deleting), you may permanently delete models via the `forceDestroy` method:
...
The `withTrashed` method may also be called when building a [relationship](/docs/orm//relationships) query:
